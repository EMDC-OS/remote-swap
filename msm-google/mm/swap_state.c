...
int drop_swapcaches_handler(struct ctl_table *table, int write,
	...
	if(write){
		...
		for (i = 0; i < MAX_SWAPFILES; i++) {
			...
			for (j = 0; j < nr; j++){
				...
				page = find_get_page(&spaces[i], j);
				__remove_mapping(&spaces[i], page, true);
...
#ifdef CONFIG_APP_AWARE
			if (swp_type(entry) == NBD_TYPE && foreground_uid){
				if(switch_start){
					atomic_inc(&switch_prefetch_hit);
					...
					atomic_inc(&after_prefetch_hit);
					//trace_printk("after prefetch hit id %d: %d \"%s\" %lx %lx\n",get_id_from_uid(foreground_uid),current->tgid,current->comm,addr,swp_offset(entry));
				}
				else
				{
					atomic_inc(&prefetch_miss);
