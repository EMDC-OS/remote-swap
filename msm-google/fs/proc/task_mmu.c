...
static int reclaim_pte_range_swap(pmd_t *pmd, unsigned long addr,
				unsigned long end, struct mm_walk *walk)
{
	pte_t *orig_pte, *pte, ptent;
	spinlock_t *ptl;
				...
				size_t count, loff_t *ppos)
{
	...
	if (mm) {
		...
		for (vma = mm->mmap; vma; vma = vma->vm_next) {
			...
			if(type==RECLAIM_SWAP){
				if(vma_is_anonymous(vma)){
					reclaim_walk.pmd_entry = reclaim_pte_range_swap;
